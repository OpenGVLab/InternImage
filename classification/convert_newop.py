...
parser.add_argument('distname', nargs='?', type=str, default=None)
...
def convert_to_newop(m):
    ...
    for k, v in m.items():
        ...
        if 'ema' in k:
            continue
        if "res_post_norm" in k:
            new_k = new_k.replace('res_post_norm', 'norm')
            print(k)
        if ".norm1_k." in k:
...
torch.save(new_model, f'./{args.distname}')
